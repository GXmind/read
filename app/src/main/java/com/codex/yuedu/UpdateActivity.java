...
public final class UpdateActivity extends Activity {
    private LinearLayout content;private TextView status,detail;private Button action;private AssetInfo latest;private File verifiedApk;private String verifiedSha;private boolean waitingPermission;
    ...
    private void build(){
        content=new LinearLayout(this);content.setOrientation(LinearLayout.VERTICAL);content.setPadding(dp(24),dp(14),dp(24),dp(28));content.setBackgroundColor(0xfffaf9f7);
        ...
        action=new Button(this);action.setText("检查更新");action.setTextSize(16);action.setTextColor(Color.WHITE);action.setAllCaps(false);action.setTypeface(Typeface.DEFAULT,Typeface.BOLD);action.setBackground(round(0xfffa4c3c,27));action.setOnClickListener(v->checkUpdate());LinearLayout.LayoutParams ap=new LinearLayout.LayoutParams(-1,dp(54));ap.setMargins(0,dp(16),0,dp(12));content.addView(action,ap);
        TextView trust=text("安全说明：应用不会静默安装；Android 系统安装器仍会再次校验应用签名并要求你的确认。",12,0xff99999e);trust.setGravity(Gravity.TOP);content.addView(trust,new LinearLayout.LayoutParams(-1,dp(58)));setContentView(content);UiInsets.standard(this,content,Color.WHITE);
    }
    ...
    private void showRelease(AssetInfo info){
        if(!VersionComparator.isRemoteNewer(info.tag,currentVersionName())){
            latest=null;
            verifiedApk=null;verifiedSha=null;
            status.setText("目前已经是最新版本");
            detail.setText("当前版本 v"+currentVersionName()+" 已是 GitHub Releases 中的最新正式版本，不需要更新。");
            setAction("重新检查",v->checkUpdate());
            return;
        }
        latest=info;status.setText("发现新版本  "+info.tag);detail.setText(info.name+"\n\n"+info.notes);setAction("安全下载并验证",v->downloadLatest());
    }
    private void downloadLatest(){if(latest==null)return;verifiedApk=null;verifiedSha=null;setBusy("正在下载更新…");detail.setText("下载完成前不会调用安装程序。");new Thread(()->{try{File base=new File(getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS),"updates");if(!base.exists()&&!base.mkdirs())throw new IOException("无法创建更新目录");File temp=new File(base,"yuedu-update.tmp");MessageDigest digest=MessageDigest.getInstance("SHA-256");long total=download(openFollowingRedirects(latest.url),temp,digest,latest.size);String sha=hex(digest.digest());if(latest.size>0&&total!=latest.size)throw new SecurityException("下载大小与 GitHub Release 不一致");if(!latest.digest.isEmpty()&&!latest.digest.equalsIgnoreCase("sha256:"+sha))throw new SecurityException("SHA-256 与 GitHub 摘要不一致");PackageInfo incoming=verifyApk(temp);File target=new File(base,UpdatePolicy.installFileName(sha,total));if(target.exists()&&!target.delete())throw new IOException("无法替换旧更新包");if(!temp.renameTo(target))throw new IOException("无法保存已验证更新包");verifiedApk=target;verifiedSha=sha;runOnUiThread(()->readyToInstall(sha,incoming.versionName,versionCode(incoming)));}catch(Exception e){runOnUiThread(()->showError("更新已拦截",friendly(e)));}}).start();}
    ...
    private PackageInfo verifyApk(File apk) throws Exception {PackageManager pm=getPackageManager();PackageInfo archive=packageInfoArchive(pm,apk);PackageInfo installed=packageInfoInstalled(pm);if(archive==null)throw new SecurityException("下载文件不是有效 APK");if(!getPackageName().equals(archive.packageName))throw new SecurityException("APK 包名不匹配");if(versionCode(archive)<=versionCode(installed))throw new SecurityException("更新版本号没有高于当前版本");Signature[] incomingHistory=signingHistory(archive),current=signers(installed);if(incomingHistory.length==0||current.length==0)throw new SecurityException("APK 签名结构异常");boolean match=false;for(Signature old:current)for(Signature candidate:incomingHistory)if(MessageDigest.isEqual(old.toByteArray(),candidate.toByteArray()))match=true;if(!match)throw new SecurityException("APK 签名证书或轮换链与当前应用不一致");return archive;}
    ...
    private void launchInstaller(){String revision=UpdatePolicy.installRevision(verifiedSha,verifiedApk.length());Uri content=new Uri.Builder().scheme("content").authority(getPackageName()+".updates").appendPath(verifiedApk.getName()).appendQueryParameter("revision",revision).build();Intent i=new Intent(Intent.ACTION_VIEW).setDataAndType(content,"application/vnd.android.package-archive").addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION|Intent.FLAG_ACTIVITY_NEW_TASK|Intent.FLAG_ACTIVITY_CLEAR_TOP);i.setClipData(ClipData.newRawUri("悦读更新 "+revision,content));startActivity(i);}
