...
public final class UpdateActivity extends Activity {
    private static final String RELEASE_API="https://api.github.com/repos/GXmind/read/releases/latest";
    ...
    private long download(HttpsURLConnection c,File out,MessageDigest digest,long declared) throws Exception {if(declared>MAX_APK_BYTES)throw new SecurityException("APK 超过允许的大小");long total=0;try(InputStream in=new BufferedInputStream(c.getInputStream());OutputStream file=new BufferedOutputStream(new FileOutputStream(out))){byte[] buf=new byte[32768];int n;while((n=in.read(buf))!=-1){total+=n;if(total>MAX_APK_BYTES)throw new SecurityException("APK 超过 300MB 安全限制");digest.update(buf,0,n);file.write(buf,0,n);}}finally{c.disconnect();}return total;}
    ...
    private void validateUrl(URL url){if(!"https".equalsIgnoreCase(url.getProtocol())||url.getUserInfo()!=null||url.getPort()!=-1||!ALLOWED_HOSTS.contains(url.getHost().toLowerCase(Locale.ROOT)))throw new SecurityException("更新地址不在安全域名白名单");}
