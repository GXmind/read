...
public class ReaderActivity extends Activity {
    ...
    private LinearLayout makeBottomPanel(){
        LinearLayout panel=new LinearLayout(this);panel.setOrientation(LinearLayout.VERTICAL);panel.setPadding(dp(24),dp(9),dp(24),dp(8));panel.setBackground(round(paperColor,28));panel.setElevation(dp(10));
    ...
    private TextView theme(String s,int paper,int ink){TextView t=text(s,13,ink);t.setGravity(Gravity.CENTER);t.setBackground(round(paper,18));t.setOnClickListener(v->applyTheme(paper,ink));LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(0,dp(44),1);p.setMargins(0,0,dp(9),0);t.setLayoutParams(p);return t;}
    private TextView modeChip(String s,boolean on){TextView t=text(s,13,on?0xfffa4c3c:0xff66615c);t.setGravity(Gravity.CENTER);t.setBackground(round(on?0xffffece9:0xfff2f1ef,18));LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(0,dp(39),1);p.setMargins(0,0,dp(10),0);t.setLayoutParams(p);return t;}
    ...
    private void editNote(){
        ...
        String quote=currentQuote();TextView excerpt=text("“"+quote+"”",13,0xff77736e);excerpt.setMaxLines(3);excerpt.setEllipsize(android.text.TextUtils.TruncateAt.END);excerpt.setPadding(dp(14),dp(9),dp(14),dp(9));excerpt.setBackground(round(0xfff4f2ef,16));box.addView(excerpt,new LinearLayout.LayoutParams(-1,dp(70)));
