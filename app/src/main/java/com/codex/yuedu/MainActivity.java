...
public class MainActivity extends Activity {
    ...
    private void render() {
        ...
        LinearLayout tabs=new LinearLayout(this);tabs.setGravity(Gravity.CENTER_VERTICAL);tabs.addView(tab("书架",true));tabs.addView(tab("阅读历史",false));tabs.addView(tab("我的笔记",false));content.addView(tabs,new LinearLayout.LayoutParams(-1,dp(48)));

        List<Book> books=LibraryStore.books(this);
        LinearLayout continueCard=new LinearLayout(this);continueCard.setPadding(dp(17),dp(14),dp(12),dp(14));continueCard.setGravity(Gravity.CENTER_VERTICAL);continueCard.setBackground(rounded(Color.rgb(255,247,244),14));
    ...
    private TextView chip(String text,boolean selected){TextView t=label(text,13,selected?ACCENT:Color.rgb(105,105,110));t.setGravity(Gravity.CENTER);t.setBackground(rounded(selected?Color.rgb(255,238,235):Color.rgb(247,247,248),16));LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(0,dp(31),1);p.setMargins(0,0,dp(8),0);t.setLayoutParams(p);return t;}

    private void addImportTile(GridLayout grid){
        LinearLayout tile=tileContainer();TextView cover=label("＋\n\n导入本地书",17,Color.rgb(125,125,130));cover.setGravity(Gravity.CENTER);cover.setLineSpacing(0,.85f);GradientDrawable bg=rounded(Color.rgb(247,247,248),10);bg.setStroke(dp(1),Color.rgb(225,225,228),dp(5),dp(4));cover.setBackground(bg);tile.addView(cover,new LinearLayout.LayoutParams(-1,dp(142)));TextView name=label("扫描手机文档",14,Color.rgb(60,60,64));name.setGravity(Gravity.CENTER);tile.addView(name,new LinearLayout.LayoutParams(-1,dp(39)));tile.setOnClickListener(v->pick());grid.addView(tile,tileParams());
    }
    private void addBookTile(GridLayout grid,Book book){
        LinearLayout tile=tileContainer();
        FrameLayout cover=new FrameLayout(this);int[] palette={0xff596b8d,0xff8b6253,0xff537a70,0xff7f668e,0xffb06e5b,0xff586d78};int color=palette[Math.abs(book.title.hashCode())%palette.length];cover.setBackground(rounded(color,10));
    ...
    private LinearLayout bottomNav(){LinearLayout nav=new LinearLayout(this);nav.setGravity(Gravity.CENTER);nav.setPadding(dp(8),dp(3),dp(8),dp(3));nav.setBackgroundColor(Color.WHITE);nav.setElevation(dp(12));LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(0,-1,1);nav.addView(navItem("▣","书架",true),p);nav.addView(navItem("▤","本地",false),p);nav.addView(navItem("✎","笔记",false),p);LinearLayout mine=navItem("○","更新",false);mine.setOnClickListener(v->startActivity(new Intent(this,UpdateActivity.class)));nav.addView(mine,p);return nav;}
